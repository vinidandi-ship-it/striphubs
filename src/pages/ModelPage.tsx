...
export default function ModelPage() {
  ...
  if (!model) {
    return <div className="rounded-xl border border-border bg-panel p-6">Model not found.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-6 rounded-2xl border border-border bg-panel p-5 md:grid-cols-[340px_1fr]">
        <img
          src={model.thumbnail}
          alt={model.name}
        ...
        <div>
          ...
          <p className="mt-4 text-sm text-zinc-400">Category: {model.category}</p>
          ...
          <a
            ...
            className="mt-6 inline-block rounded-lg bg-accent px-6 py-3 font-bold text-white"
          >
            Watch {model.name} Live
          </a>
        </div>
      </section>
    </div>
