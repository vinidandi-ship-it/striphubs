...
export const selectProviderByRotation = (): ProviderId => {
  ...
  if (stripchatRatio < 0.6) {
    selected = 'stripchat';
  } else if (chaturbateRatio < 0.4) {
    selected = 'chaturbate';
  } else {
...
export const selectProviderByPerformance = (): ProviderId => {
  ...
  const stripchatWeight = Math.max(30, AFFILIATE_PROVIDERS.stripchat.weight * (stripchatCtr / (stripchatCtr + chaturbateCtr || 0.5)));
